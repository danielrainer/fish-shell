...
def main():
    ...
    def print_result(arg, result, color, duration=None, suffix=None):
        duration_str = "" if duration is None else f" {str(duration_ms).rjust(max_expected_digits_duration)} ms"
        suffix_str = "" if suffix is None else f"\n{suffix}"
        print(f"{arg.ljust(longest_test_name_length)}  {color}{result}{RESET}  {duration_str}{suffix_str}")
...
def run_test(f, arg, script_path, args, def_subs, lconfig, fishdir) -> TestResult:
