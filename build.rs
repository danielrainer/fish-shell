...
/// If it is a symlink, the path of the link will be included, as well as the result of recursing
/// on its target.
/// If it is a directory, the function recurses on each entry and combines the results into the
/// returned iterator.
fn ls_recursive<P: AsRef<Path>>(path: P) -> impl Iterator<Item = PathBuf> {
    fn walk(path: PathBuf) -> Box<dyn Iterator<Item = PathBuf>> {
        ...
        if path.is_symlink() {
            return Box::new(
                [path.clone()]
                    .into_iter()
                    .chain(walk(path.read_link().unwrap())),
            );
        }
        if path.is_dir() {
            return Box::new(
...
/// For symlinks, both the link and the target are considered.
